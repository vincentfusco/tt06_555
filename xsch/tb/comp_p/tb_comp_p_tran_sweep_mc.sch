...
let v_th = vector(mc_runs)
let v_tl = vector(mc_runs)
let v_hyst = vector(mc_runs)
let v_offset = vector(mc_runs)
let design_point = vector(mc_runs) ; lets me plot quantities vs mc run
...
while run <= mc_runs
	...
	let v_th[run] = v_cross_lh-0.6
	let v_tl[run] = v_cross_hl-0.6
	let v_hyst[run] = v_th[run] - v_tl[run]
	let v_offset[run] = v_th[run] + v_tl[run]
...
plot tran1.v(out) tran2.v(out) tran3.v(out) tran4.v(out) tran5.v(out) tran6.v(out) tran7.v(out) tran8.v(out) tran9.v(out) tran10.v(out) tran1.v(vinp) tran1.v(vinn)

wrdata tb_comp_p_tran_sweep_mc_vth.txt v_th vs design_point
wrdata tb_comp_p_tran_sweep_mc_vhyst.txt v_hyst vs design_point
